...
function redrawCanvas() {
    clearWhite();
    // draw all bars, skip the one currently dragged (so we can draw it at cursor)
...
function onMouseDown(e) {
    ...
    if (dragIndex !== -1) {
        ...
        if (infoElem) infoElem.innerText = "Dragging kotak " + data[dragIndex] + "...";
        redrawCanvas();
    }
}
...
function onMouseUp(e) {
    ...
    if (dropIndex !== -1 && dropIndex !== dragIndex) {
        ...
        if (infoElem) infoElem.innerText = "Drag dibatalkan.";
    }
    dragIndex = -1;
    redrawCanvas();
}
